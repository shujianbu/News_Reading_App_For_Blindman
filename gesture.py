...
class GestureAnalyzer(object):
    ...
    def recognize(self, contours): 
        ...
        if self.isFist(max_area_n, mean_depth):  
            ges = 'Fist' 
        elif self.isPalm(max_area_n, mean_depth): 
            ges = 'Palm'
        elif self.isScissors(max_area_n, mean_depth): 
            ges = 'Scissors'    
        else: 
            ges = 'Uncertain'
    ...
    def isFist(self, area, depth): 
        return GesCons.FIST.DEPTH_L < depth < GesCons.FIST.DEPTH_U and \
               GesCons.FIST.AREA_L  < area  < GesCons.FIST.AREA_U

    def isPalm(self, area, depth): 
        return GesCons.PALM.DEPTH_L < depth < GesCons.PALM.DEPTH_U and \
               GesCons.PALM.AREA_L  < area  < GesCons.PALM.AREA_U 

    def isScissors(self, area, depth): 
        return GesCons.SCISSORS.DEPTH_L < depth < GesCons.SCISSORS.DEPTH_U and \
               GesCons.SCISSORS.AREA_L  < area  < GesCons.SCISSORS.AREA_U
